...
def pre_order_info_upload(data,buyer_id):
    try:
        ...
        cursor.execute(
                "INSERT INTO pre_order_info(order_uuid, seller_id, buyer_id, product_id, order_amount, remark_message, order_time, total_price) VALUES(%s, %s, %s, %s, %s, %s, %s, %s )",param
            )
        con.commit()
        return True
    # except Exception as err:
...
def get_username(user_id):
    try:
        ...
        cursor.execute(
                "SELECT username as user FROM members WHERE id = %s",(user_id,)
            )
        response = cursor.fetchone()
        return response
    except Exception as err:
