...
def get_seller_finish_info(seller_id):
    try:
        ...
        cursor.execute(
            "SELECT  members.username, img.image_url, pre.*, a.owner_pre_site, a.id, a.product_name, a.product_price, a.product_amount FROM finish_trade_info as pre JOIN product_info as a ON pre.product_id = a.id JOIN product_images as img ON img.product_id = a.id JOIN members ON members.id = a.owner_id  WHERE pre.seller_id = %s GROUP BY a.id",
            (seller_id,),
        )
        response = cursor.fetchall()
        return response
